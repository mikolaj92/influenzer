# github_pack

Survey JSON → facts + ship/tryable flags, or silence.

Many public signals; only a ship a stranger can try becomes facts.
Patch/typo/chore is silence. A waitlist is not a ship. A window of
merged PRs is changelog, not a tryable ship. Tryable is a README+URL
heuristic, not a launch. Look does not clone or run the project to
prove it. Code in look (theirs or ours) is untrusted.

Does **not**: call `gh`, write SQLite, tick, publish, or know drafts,
arenas, or `state.db`. Does not clone or launch the project.
`import github_pack` does not import Influenzer and does not import
`github_survey`.
