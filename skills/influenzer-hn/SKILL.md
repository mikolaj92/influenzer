...
Show HN is three fields or silence: `Show HN: …`, URL in the URL field, first comment as backstory. Not a blog, not a waitlist, not "please upvote". Human backstory, not a press release. Title + URL without a first comment is not Show HN. A lab notebook is not Show HN: `story_kind` exploration / decision / failure without a tryable ship does not sit here. Workshop or silence. Seminar only when a stranger can click and run it.

## Wave

- Title starts with Show HN and a working demo. One line, not a blog. No waitlist, no blog-as-Show. Press-release tone (we’re excited, announcement, unveiling, delighted to share) is kill, not Show HN. `story_kind` exploration / decision / failure does not sit: workshop or silence. Not live.
