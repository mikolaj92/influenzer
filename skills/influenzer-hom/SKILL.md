...
- Look at GitHub on a coarse cadence (about weekly), one story or silence — not every tick. Declare the look with `influenzer watch set --project-id ID --repo owner/name`. The interval loop still scores; when that watch is due it runs `influenzer pass` once (listen and look). `--once` stays score-only unless `--pass-if-due`. `influenzer pass --project-id ID --repo owner/name` is one CMO look: inbound replies, then scan-due, then score pending briefs, at most one wearable angle. `influenzer brief scan-due --project-id ID --repo owner/name` (or `brief scan --if-due`) is the scan-only slice. `influenzer feedback --project-id ID --repo owner/name` (or the declared watch) is the listen-only slice; bots and LGTM stay silent; it does not post replies. Verdict stays the gate.
...
influenzer angle
influenzer verdict hold
# or one CMO look:
influenzer pass --project-id app-1 --repo owner/name
```
