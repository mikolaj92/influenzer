    ...
    def test_roadmap_is_undressable_even_when_score_says_draft(self) -> None:
        ...
        for text in calendars:
            with self.subTest(text=text):
                ...
                self.assertIsNone(payload["body"])
                dumped = json.dumps(payload)
                self.assertNotIn("Coming Q3", dumped)
                self.assertNotIn("shipping soon", dumped)
                self.assertNotIn("on the roadmap", dumped)
                self.assertNotIn("Show HN:", dumped)

    def test_product_copy_without_a_roadmap_can_still_dress(self) -> None:
        brief = _ship_brief(
            preferred_arena=ArenaId.HN,
            facts=(
                Fact(text="Local tick scores briefs and emits a draft", artifact_url=SHIP_PR),
        ...
        decision = apply_brief(brief)
        assert decision.draft is not None
        self.assertIn("as soon as the install finishes", decision.draft.body)
        self.assertTrue(decision.draft.body.startswith("Show HN:"))
        self.assertNotIn("Coming Q3", decision.draft.body)
        self.assertNotIn("on the roadmap", decision.draft.body.lower())
