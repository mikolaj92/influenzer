...
4. Ingest **briefs** (many facts), or run `influenzer brief scan --project-id app-1 --repo owner/name` to pack public GitHub signals into 0 or 1 pending brief. On a weekly-ish clock, `influenzer brief scan-due` (or `brief scan --if-due`) does that only when due. `influenzer-tick-all` (or `influenzer-tick --once`) scores them: kill, changelog-only, or one-arena draft. Borderline briefs stay silent. Drafts are not published. Scan never publishes. Tick does not survey GitHub.
