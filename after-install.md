...
4. Ingest **briefs** (many facts), or run `influenzer brief scan --project-id app-1 --repo owner/name` to pack public GitHub signals into 0 or 1 pending brief. `influenzer feedback --project-id app-1 --repo owner/name` packs inbound issue/PR comments the same way (never posts a reply). `influenzer pass --project-id app-1 --repo owner/name` is one CMO look: it starts a new GitHub scan only on Monday (Europe/Warsaw), if no look has completed for that Monday; an interrupted scan may resume on a later day. It then scores pending briefs and may emit at most one angle. `influenzer brief scan-due` (or `brief scan --if-due`) is the scan-only slice. `influenzer-tick-all` (or `influenzer-tick --once`) scores them: kill, changelog-only, or one-arena draft. `--once` does not scan unless `--pass-if-due`. Borderline briefs stay silent. Drafts are not published. Scan never publishes. Verdict stays the gate.
