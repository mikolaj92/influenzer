...
`influenzer feedback --project-id ID --repo owner/name` (or the declared watch) reads public issue/PR comments through the same `gh` subprocess and stores **at most one** pending brief (`source=github-feedback`), or stays silent. Bots, LGTM, and empty thanks fail closed. A real question, bug, or pushback becomes facts. Feedback does not publish, does not auto-post replies, and does not choose a social angle; `tick-all` still scores pending briefs.

`brief scan-due` (or `brief scan --if-due`) is the same compose **only when due**: a pending brief or unprocessed social draft is silence; a successful look (or github-scan brief) newer than 7 days (overridable) is `not due` and does not call `gh`. `influenzer pass --project-id ID --repo owner/name` is **one CMO look**: listen for replies (`hom_feedback`), then that scan-due, then score pending briefs, then at most one wearable angle. One story at a time — a feedback brief silences the ship scan. Verdict stays the gate. Declare the look with `influenzer watch set --project-id ID --repo owner/name`. The interval loop still scores every time; when that watch exists and scan-due would consider it due, it runs `hom_pass` once (listen and look). `--once` stays score-only unless `--pass-if-due`.
...
- **hom_pass** — one CMO look: `hom_feedback` → `scan_due` → tick (score pending briefs) → `hom_outbox` (at most one angle). Reuses those functions; does not copy survey/pack/admit/score/dress/outbox or `github_feedback`. Does not verdict, publish, enable live, call `gh`, know Heimdall, or run every tick interval. Host compose is `influenzer pass --project-id ID --repo owner/name`. The interval loop may invoke this once when a declared watch is due. Fala may run `python3 -m influenzer.hom_pass`.
