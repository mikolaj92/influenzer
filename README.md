...
- **hom_watch** — declare one project → one repo (`influenzer watch set` / `watch show`). Persisted in `state.db`. The interval tick reuses `scan_due_reason` and runs existing `hom_pass` when due; otherwise it only scores. Look does not launch or run that project. `--once` does not scan unless `--pass-if-due`. No Fala organ.
