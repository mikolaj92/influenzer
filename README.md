...
- **github_pack** — survey JSON → facts + ship/tryable, or silence. Tryable is a README+URL heuristic, not a run of the project. Does not call `gh`, write SQLite, or tick. See [`github_pack/README.md`](github_pack/README.md).
...
- **hom_watch** — declare one project → one repo (`influenzer watch set` / `watch show`). Persisted in `state.db`. The interval tick reuses `scan_due_reason` and runs existing `hom_pass` when due; otherwise it only scores. `--once` does not scan unless `--pass-if-due`. Look does not launch the project. Tryable stays a README+URL heuristic. No Fala organ.
