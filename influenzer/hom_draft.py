...
from influenzer.host import looks_like_captcha_challenge, looks_like_geo_block
...
def _clickable_urls(brief: Brief) -> tuple[str, ...]:
    ...
    for fact in brief.facts:
        ...
        if (
            ...
            and not is_linktree_host_url(url)
            and not is_cloud_drive_host_url(url)
...
def _undressable_blob(bits: CopyBits) -> bool:
    return (
        ...
        or looks_like_linktree(bits.blob)
        or looks_like_cloud_drive(bits.blob)
...
def _dress_hn(bits: CopyBits, score: Score) -> str | None:
    ...
    if (
        ...
        or is_linktree_host_url(url)
        or is_cloud_drive_host_url(url)
...
def dress_brief(brief: Brief, score: Score, *, now: str | None = None) -> Draft | None:
    ...
    if (
        ...
        or looks_like_linktree(body)
        or looks_like_cloud_drive(body)
