...
def _docs_typo_chore_bits(bits: CopyBits) -> bool:
    return looks_like_docs_typo_chore_window((bits.one_liner, *bits.rest))
...
def _dress_hn(bits: CopyBits, score: Score) -> str | None:
    ...
    if looks_like_merged_pr_fact(title_src) or _merge_log_bits(bits) or _docs_typo_chore_bits(bits):
        return None
