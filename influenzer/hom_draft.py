...
def _dress_hn(bits: CopyBits, score: Score) -> str | None:
    ...
    if not backstory:
        return None
    if looks_like_waitlist(backstory) or looks_like_event(backstory) or looks_like_calendar_filler(backstory) or looks_like_counter_thanks(backstory) or looks_like_fog(backstory) or looks_like_founder_journal(backstory) or looks_like_lead_magnet(backstory) or looks_like_solicit_gesture(backstory):
        return None
