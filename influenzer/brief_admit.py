...
def admit_pack(
    ...
    *,
    ...
    hn_url = hn_submission_url_key(facts_raw)
    if hn_url is not None and hn_url in repo.hn_submission_url_keys():
        return host_silence(SAME_HN_URL_REASON, project_id=project_id, repo_slug=slug)
    ...
    if looks_like_fomo(fact_blob):
        return host_silence(FOMO_REASON, project_id=project_id, repo_slug=slug)
    if looks_like_meme(fact_blob):
    ...
    if looks_like_cloud_drive(fact_blob):
        return host_silence(CLOUD_DRIVE_REASON, project_id=project_id, repo_slug=slug)
