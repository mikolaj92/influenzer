...
`owner/name`. Missing `gh`, auth failure, a private repo, an empty
window, or a repo with no tree / no README is a silent envelope, not a
crash. README/comments/JSON over the hard byte limit is an empty look,
not a feast. 50MB in `state.db` is silence. The loop lives. Look does
not run the project.
